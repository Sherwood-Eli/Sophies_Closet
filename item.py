...
class Item(Clothing_Unit):
	...
	def get_image_icons(self):
		...
		for entry in image_ids:
			...
			image = ui.Image.named("images/item_images/"+image_id+".PNG").with_rendering_mode(ui.RENDERING_MODE_ORIGINAL)
	...
	def add_image(self, sender):
		self.image_selector = Image_Selector("item_images", "item_thumbnails", "", self.save_new_image, self.outfit_saver, False)
	...
	def get_link_icons(self):
		...
		ON outfits.outfit_id = outfit_items.outfit_id
		WHERE item_id="{}"
		'''.format(self.id)
		cursor.execute(sql)
	...
	def add_link(self, sender):
		self.link_selector = Search_View("Search Clothing Outfits", "outfits", "outfit_id", "outfit_name", "outfit_image_id", "outfit_images", "outfit_thumbnails", self.choose_link, self.outfit_saver)
	...
	def choose_link(self, sender):
		...
		'''.format(int(link_id), int(self.id))
	...
	def open_link(self, sender):
		...
		Outfit(sender.name, title, self.category, True, self.outfit_saver)
	...
	def get_category_icons(self):
		...
		sql = '''
		SELECT item_categories.category_id, item_categories.category_name, item_categories.photo_path
		FROM "item_categories"
		INNER JOIN "category_items"
		ON item_categories.category_id = category_items.category_id
		WHERE item_id="{}"
		'''.format(self.id)
		cursor.execute(sql)
	...
	def add_category(self, sender):
		self.category_selector = Search_View("Search Item Categories", "item_categories", "category_id, category_name, photo_path", "category_name", None, None, "background_thumbnails", self.choose_category, self.outfit_saver)
	...
	def choose_category(self, sender):
		...
		#need this to keep "+" button at the end
		new_category = self.create_category_icon(category_id, category[0][0], category[0][1])
