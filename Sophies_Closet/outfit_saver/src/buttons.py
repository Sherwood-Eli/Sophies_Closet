...
class Remove_Button:
	def __init__(self, action, outfit_saver):
		...
		self.button = ui.Button(action = action, background_color="f0fff5")
		self.button.width=(s_width*.1)
		self.button.height = self.button.width
		self.button.corner_radius = self.button.width/2
		self.button.center = (outfit_saver.screen_width*.5, outfit_saver.screen_height*.96)
		self.button.name = "f"
		self.button.border_width = 2
		self.button.add_subview(image_view)
...
class Title_Button:
	def __init__(self, title, action, s_width, s_height):
		self.button = ui.Button(action=action, width=(s_width*(3/5)), height=(s_width/10), font = ('<system-bold>', 20), border_width=2, background_color="f0fff5")
